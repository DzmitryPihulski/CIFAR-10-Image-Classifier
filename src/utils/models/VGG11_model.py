...
class ConvBlock(nn.Module):
    ...
    def __init__(self, in_channels: int, out_channels: int, pool: bool = False):
        ...
        layers: List[nn.Conv2d | nn.BatchNorm2d | nn.ReLU | nn.MaxPool2d] = [
...
class VGG11(nn.Module):
    ...
    def __init__(self):
        ...
        self.fc1 = nn.Sequential(
            nn.Dropout(0.5), nn.Linear(4 * 4 * 512, 2048), nn.ReLU()
        )
        self.fc2 = nn.Sequential(nn.Dropout(0.5), nn.Linear(2048, 2048), nn.ReLU())
        self.fc3 = nn.Sequential(nn.Linear(2048, 10))
