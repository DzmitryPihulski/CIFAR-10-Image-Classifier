...
def train_model(
    ...
    for epoch in epochs:
        ...
        for i, data in enumerate(train_loader, 0):
            ...
            if i % 500 == 499:
                print(
                    f"Current time of execution: {round(time.time() - current_time, 0)} s, epoch: {epoch}/{epochs[-1]}, minibatch: {i + 1:5d}/{len(train_loader)}, running loss: {running_loss / 500:.3f}"
                )
                running_loss = 0
    ...
    plt.annotate(  # type: ignore
        text=f"Min loss after {losses_val.index(min(losses_val))+1} epohs",
    ...
    plt.title(f"Train and Val loss after {len(losses_train)} epochs")  # type: ignore
    plt.xlabel("Epoch")  # type: ignore
    plt.ylabel("CrossEntropyLoss")  # type: ignore
    plt.legend(loc="upper left")  # type: ignore
    plt.savefig(f"src/data/images/{model_name}.png")  # type: ignore
...
train_set.load_batch_transformed(
    "src/data/cifar-10-batches-py/data_batch_1", transform_pipeline
)
train_set.load_batch("src/data/cifar-10-batches-py/data_batch_2")
train_set.load_batch_transformed(
    "src/data/cifar-10-batches-py/data_batch_2", transform_pipeline
)
train_set.load_batch("src/data/cifar-10-batches-py/data_batch_3")
train_set.load_batch_transformed(
    "src/data/cifar-10-batches-py/data_batch_3", transform_pipeline
)
train_set.load_batch("src/data/cifar-10-batches-py/data_batch_4")
train_set.load_batch_transformed(
    "src/data/cifar-10-batches-py/data_batch_4", transform_pipeline
)
...
validation_loader = torch.utils.data.DataLoader(  # type: ignore
    validation_set, batch_size=32, shuffle=False, pin_memory=True
)
...
# Train models
train_model(ConvNet_1(), "ConvNet_1", train_loader, validation_loader, test_loader)
train_model(ConvNet_2(), "ConvNet_2", train_loader, validation_loader, test_loader)
train_model(ResNet18(), "ResNet18", train_loader, validation_loader, test_loader)
train_model(VGG11(), "VGG11", train_loader, validation_loader, test_loader)
